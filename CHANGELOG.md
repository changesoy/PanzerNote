...
- **行内格式 toggle 无法闭环**（同一验收发现）：包裹后选区丢失、光标停在「文字」与闭标记之间，再按一次同一键会插入若干星号、破坏结构；想取消只能手动把两侧星号一起选中。现包裹后**保留核心文字的选区**，再按一次即走剥壳分支；只选内部文字也生效（选区外侧紧贴同类标记时吸收**整段同类标记串**，避免斜体把 `**粗体**` 削成 `*粗体*`）；光标正贴成对标记（`**|**`、`**文字|**`）时按一次即取消该对
...
- **文件树内拖拽「不问就搬」**（人工验收发现）：`QFileSystemModel` 在 `readOnly(False)` 后能自己完成落盘（rename / copy），树内拖拽因此既不询问移动还是复制、**也不做图片资源迁移**，与「拖标签页」两条路径行为不一致。现树内拖拽由视图接管，走与标签拖拽同一条请求链路（询问 → 移动 / 复制请求 → 应用层迁移），并拦掉原地放下与「文件夹拖进自己的子孙目录」
...
- **预览不跟随编辑区「限制行宽」**（人工验收发现）：编辑区「限制行宽」下一切内容都严格限制在面板宽度内、不出现横向滚动，而预览里长代码行只在代码块内横向滚动、宽表格会把整页顶宽，两边观感不一致。现 `Editor` 新增 `wrap_mode_changed` 信号，预览据此给 `body` 加 `limit-width`（代码块折行、表格单元格可断行），「不换行」模式维持原样
