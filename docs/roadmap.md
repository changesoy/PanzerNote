...
在 Markdown 中插入图片已可用：图片落盘到笔记同级的 `PanzerNote_assets/`，文档写入相对路径。配套的资源归属与断链恢复仍在推进：

- **managed-asset ledger**：`data/config/image_assets.json` 隐藏恢复索引（名称 / 最后已知路径 / sha256 / 尺寸 / 来源），只作「最后已知线索」，不取代 Markdown 与文件系统作为真实状态
- **缺失检测**：文档打开 / 建立预览时做 Python 侧只读扫描，经状态栏或小秘书**非打断**提示，不与渲染循环绑定
- **资源迁移**：程序内移动 / 另存为 / 复制文档时，按「确认独占 → Move；确认共享 → Copy」搬运图片，避免制造断链
- **外部移动恢复**：在资源管理器里移动笔记后，基于 ledger + 真实引用扫描 + hash 校验恢复断链；证据不足时不猜，交给用户判断
