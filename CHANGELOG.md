...
- **文件树显示图片 + 图片查看器**：文件树此前只列文本扩展名，图片在树里完全不可见；现在 png/jpg/jpeg/gif/webp/bmp/svg 及下述更多格式都会列出，双击**在编辑区以标签页形式打开**（与文档标签同族、可关闭、不占额外窗口；同一张图重复双击聚焦既有标签）；默认适应窗口（只缩不放，小图按原始像素显示），单击在「适应窗口 ⇄ 原始大小」间切换，大图可滚动，底部状态行显示像素尺寸与当前缩放
