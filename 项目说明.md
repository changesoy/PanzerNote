...
| **打字奖励**         | ✅ 完成   | `textChanged` 信号接入 → 字符计数 → 递减收益算法 → 资源奖励（1:1:1:0.2 燃料/弹药/钢材/铝材） |
...
│   │   ├── menu_builder.py         # 菜单构建器（已接入ShortcutManager，所有菜单项通过manager.register()注册）
│   │   └── shortcut_manager.py     # 快捷键管理器（注册/冲突检测/自定义/持久化）
│   │
│   ├── editor/                     # ── 编辑器模块 ──
│   │   ├── __init__.py
│   │   ├── editor.py               # 核心编辑器（行号、缩略图、语法高亮、自动缩进）
│   │   ├── editor_tabs.py          # 多标签管理（打开/保存/关闭/编码检测/拖拽/文件移动/安全读写）
│   │   ├── editor_actions.py       # 行操作/大小写转换/JSON/XML格式化（Mixin）
│   │   ├── auto_pair_handler.py    # 括号/引号自动配对（Mixin，支持中英文标点）
...
│   │   ├── markdown_preview.py     # Markdown分屏（PreviewBrowser + 代码块高亮 + 本地图片 + 异步渲染）
│   │   ├── minimap.py              # 代码缩略图（块级缓存渲染 + 视口指示器 + 鼠标导航）
...
| 模块                          | 职责                                                     |
| ----------------------------- | -------------------------------------------------------- |
| `editor/editor.py`            | 核心编辑器（行号、缩略图、语法高亮、自动缩进、虚拟滚动） |
| `editor/editor_actions.py`    | 行操作、大小写转换、JSON/XML格式化（Mixin）              |
| `editor/auto_pair_handler.py` | 括号/引号自动配对（Mixin，支持中英文标点）               |
...
```
src/__init__.py (__version__ = "1.6.5")
...
- `get_version_tuple()` — 返回版本号元组，如 `(1, 6, 5)`
...
**代码现状**：已完成接入。`editor_tabs.py` 新增 `chars_typed` 信号，在 `_on_text_changed()` 中计算字符增量并发射；`MainWindow._on_chars_typed()` 实现递减收益算法；`GameEngine.add_typing_reward()` 将奖励转化为四项资源。
...
_本文档基于 PanzerNote v1.6.5 源码生成，涵盖全部 Python 源文件和游戏数据 JSON。_
