...
class Editor(AutoPairHandlerMixin, EditorActionsMixin, QPlainTextEdit):
    """文本编辑器"""
    ...
    def __init__(self, config: Config, parent=None):
        ...
        self._init_minimap()
        self._virtual_scroll = VirtualScrollManager(self)
    ...
    def set_file_type(self, filepath_or_ext: str):
        ...
        self._virtual_scroll.set_highlighter(self._highlighter)
    ...
    def load_content(self, content: str):
        """加载文本内容，大文件自动启用虚拟滚动"""
        if not self._virtual_scroll.load_content(content):
    ...
    def goto_line(self, line: int):
        ...
        if block.isValid():
            ...
            self.setTextCursor(cursor)
            self.centerCursor()
            if self._virtual_scroll.is_active():
                self._virtual_scroll.goto_line(line)
