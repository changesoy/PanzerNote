...
- **跨文件搜索编码兜底**：搜索此前硬编码按 UTF-8 读取，GBK / UTF-16 文件里的中文**永远搜不到**（假阴性）。现复用文档打开的解码链（`[原编码] + utf-8 + gbk + utf-16` 严格尝试，UTF-16 BOM 文件免于二进制误判），四种编码的中文均可命中
- **跨文件搜索单文件上限**：单文件按「文字重量」1 MB 上限（剔除内嵌 base64 图片 payload 后计算），超限文件跳过不读，状态栏提示跳过数量——不再因个别超大文件拖慢整轮扫描
- **大文件性能路径默认启用**（原 Wave 4 E3/E4 需手动开 flag）：达阈值（≥1 万行）自动走大文件模式（延迟加载 / 延迟高亮 / 降级补全 / 折叠 / Minimap / 预览），状态栏新增「大文件」指示标签（悬停显示降级说明，明暗主题自动适配）；显式关闭过的用户不受新默认值影响；`virtual_scroll` / `lazy_loading` 两个死 flag 已删除
- **预览后端故障可见诊断**：WebView2 运行时缺失或初始化失败时不再静默降级到纯日志——状态栏右侧出现「预览不可用」常驻指示（悬停显示原因），点击弹出诊断对话框（具体原因 + 修复指引 + 打开下载页）
- **新增「视图 → 刷新预览」**（`Ctrl+Shift+R` / 命令面板）：手动触发当前文档预览重渲染；大文件占位页的文案指向该入口
- **新增「工具 → 迁移旧图片目录...」**（命令面板可搜）：一次性把历史遗留的旧 `assets/` 目录迁入新约定 `PanzerNote_assets/` 并重写引用。预演确认制——先出完整变更清单（移动 / 改名 / 重写哪些文档），确认才动文件；采用「复制 → sha256 校验 → 删源」保底，任一步失败源文件保持原样；未被引用的图片一并迁入（之后可用「清理未使用的图片」反悔式删除）；仅被「打开且未保存」文档引用的图片整体跳过（防断链）；每文档按检测出的原编码写回（GBK 文件仍以 GBK 落盘），迁空后清理旧目录
- **菜单信息架构重组**：新增「工具」菜单（位于视图与设置之间），原「编辑 → 图片资源」「编辑 → Markdown 编辑」条目整体迁入，「转到行」并入「编辑 → 行操作」；命令面板分组同步调整（25 个条目迁至「工具」组），菜单文本「Markdown 编辑」等处的路径描述同步修正

**修复**

- **行尾切换可撤销**：状态栏点击 LF/CRLF 切换后 `Ctrl+Z` 无法撤销（切行尾产生的空撤销项挡路，还可能误撤此前的文本编辑）。现行尾变更在 Document 级单独记账并与文本撤销栈排序融合：撤销按真实时间顺序回退（先撤其后的文本编辑、再回退行尾），重做按发生顺序正向回放；右键菜单「撤销」同一路径；保存成功后清空行尾历史（已落盘的变更不再可撤销）
- **行尾来回切换不再累积脏状态**：行尾脏改为派生量（当前行尾 ⇄ 已落盘基线），CRLF→LF→CRLF 切回原值即自动回干净；保存途中切换行尾的脏信号不再漏发
- **文件树拖拽落点提示**：此前「把文件拖回它自己所在的文件夹」与「把文件夹拖进自己的子目录」照样高亮落点行，松手却没有任何动作（提示过度承诺）；而拖到列表下方的**空白处**（确实会落进笔记库根目录）反倒毫无反馈——树根不占一行，行高亮画不出来。现落点提示只在该落点真的会落盘时才点亮，空白处则把最后一行下方整块点亮

**性能：感知性能批（大文件打开 / 切主题卡顿治理）**

- **行号区重排风暴**：主题切换期间 `setViewportMargins` 被高频冗余调用（单次切换 84,787 次）触发整窗重排——现已幂等化（相同边距直接跳过）
- **主题切换只重着色可视区**：大文件 + 延迟高亮激活时，切主题不再整篇 `rehighlight()`（O(全文) 改 O(可视区)），改由延迟高亮管理器对可视块增量重着色
- **大文件暂停预览渲染**：大文件模式下打开 / 编辑不再触发预览全量渲染，显示纯 CSS 变量驱动的占位页（明暗主题自动适配），需要时用「刷新预览」手动渲染；切主题只更新 CSS 变量、不重渲染，**结构上杜绝旧主题代码块配色残留**
- **折叠折叠区包裹去二次方**：`_wrap_fold_sections` 从逐位置插入（O(ops × len)）改为单趟升序拼装（等价于逆序插入字节），15 万行文档从 28.5 s 降到 41 ms
- 实测：2 万行文档切主题 30.3 s → ~1.5 s；15 万行打开 >40 s → 25.7 s、切主题 >40 s → ~14.2 s（剩余成本大头为 markdown-it 单遍渲染，属第三方库固有）

**兼容性说明**

- `large_file_mode` 默认值 False → True：大文件模式带来的降级（补全 / 折叠 / Minimap / 预览自动刷新 / 缩略图）在 ≥1 万行的文档上默认生效；设置 → 编辑器 → 大文件可关闭回退。曾在设置里显式存过 `false` 的用户不受影响
- 大文件模式下预览不自动渲染（占位页 + 手动「刷新预览」）为**设计内行为**；手动渲染的结果在切标签后会回到占位页
- 菜单结构变化：原「编辑 → 图片资源 / Markdown 编辑」的用户若依赖 muscle memory 需要适应新「工具」菜单位置；快捷键不变
- 无用户数据格式变更：笔记、设置与存档 schema 均不变；迁移命令不改变图片内容，仅移动位置与改写引用路径

## v2.5.0

**新增：图片工作流收尾（行为变化）**

- **文件树显示图片 + 图片查看器**：文件树此前只列文本扩展名，图片在树里完全不可见；现在 png/jpg/jpeg/gif/webp/bmp/svg 及下述更多格式都会列出，双击**在编辑区以标签页形式打开**（与文档标签同族、可关闭、不占额外窗口；同一张图重复双击聚焦既有标签）；默认适应窗口（只缩不放，小图按原始像素显示），单击在「适应窗口 ⇄ 原始大小」间切换，大图可滚动，底部状态行显示像素尺寸与当前缩放；图片标签与其他标签一样**可拖拽跨分屏迁移**
- **可查看格式大幅扩展**：Qt 原生（png / jpg / jpeg / gif / webp / bmp / tif / tiff / ico / tga / pbm / xbm / xpm 等）、HEIF / HEIC（新增 `pillow-heif`）、AVIF（Pillow 内置解码，无新增依赖）。**相机 RAW 不在支持范围**：解码需 LibRaw（`rawpy`），其传递依赖 `numpy` 会带来约 54 MB 包体，成本与收益不成比例，故整体砍掉
- **查看全程只读**：字节经 FileGuard 读取、内存中解码，不写回、不重编码、不改动源文件；竖拍照片按 EXIF 方向显示（方向纠正只作用于内存中的显示，文件字节不变）
- **非常规格式可插入文档**：HEIF / TIFF 等预览渲染不了的格式也允许插入——落盘前先转码成 PNG（带透明通道）或 JPEG（照片，质量 92），保证插入后预览与导出能正常显示；源文件只读不变，落盘名换成转码后的真实扩展名（`shot.heic` → `shot.jpg`）
- **零拷贝引用**：把当前文档 `PanzerNote_assets/` 里的图片再次插入同一文档时，只新增一条引用、不再复制新副本（文件管理器里复制图片再粘贴的常见操作不再堆冗余文件）；其他文档或其他位置的图片仍复制落盘，保持各文档自包含
- **恢复缺失图片的兜底扫描**：索引里没有线索时（旧图片、手动拷贝进来的图片从未入索引），在「工作区 + 文档所在目录」范围内按文件名扫描 `PanzerNote_assets/` 找候选，唯一命中即可恢复；索引有线索但文件已删或内容已改仍**不猜**，保持原有保守策略
- **新增「清理未使用的图片」命令**（编辑菜单 / 命令面板）：扫描范围内所有 `PanzerNote_assets/` 里**未被任何 Markdown 引用**的图片，弹预览清单（按目录分组、显示大小）由用户勾选——**默认全部不勾选**，确认后移入回收站（可还原），并同步清理图片索引记录
...
- AVIF 由 Pillow 内置解码，无需额外依赖；相机 RAW 因需 LibRaw + numpy（约 54 MB）而未纳入
- 无用户数据格式变更：笔记、设置与存档 schema 均不变；`PanzerNote_assets/` 与 `image_assets.json` 约定不变
- 图片格式能力分三档且口径统一（见 `src/editor/image_formats.py`）：可渲染（Chromium，即落盘目标）⊂ 可查看 ⊂ 可插入（= 可查看，非可渲染者转码）；`.pdf` 虽被 Qt 视作图片格式，但不作为图片处理

**新增：高频 Markdown 编辑辅助（行为变化）**

- **列表与引用续写**：Markdown 行尾回车自动续写任务（`- [ ] `）/ 无序（`- `）/ 有序列表前缀（含前导缩进，有序列表序号自动递增，源码序号与渲染结果一致）；空列表项回车取消标记；`> ` 引用块同样续写并支持嵌套，空引用行回车取消
- **任务勾选**：`Ctrl+Alt+T` 在 `[ ]` ⇄ `[x]` 间切换，普通列表项自动补勾选框；非列表行不响应（`Ctrl+Shift+T` 已被「重新打开关闭的标签页」占用，故改绑）
- **表格编辑**（「编辑 → Markdown 编辑 → 表格」/ 命令面板）：上/下插入行（表头下智能落位）、删行（表头与分隔行受保护）、左/右插入列（分隔行自动补齐）、删列（单列保护）；Tab / Shift+Tab 在单元格间导航（自动跳过分隔行，末格 Tab 在表尾新建行，表头前 Shift+Tab 回落减缩进）；「格式化对齐」按最宽单元格对齐管道符，中文按 East Asian 显示宽计算
- **行内格式**：`Ctrl+Alt+B` 加粗、`Ctrl+Alt+I` 斜体、``Ctrl+Alt+` `` 行内代码、`Ctrl+Alt+K` 链接；选中文字包裹标记，无选中插入骨架且光标落在标记之间；再次按同一键剥掉标记（斜体不误剥粗体标记，粗斜体剥层还原粗体；链接选中 `[文本](url)` 整体时还原为文本）
- **标题级别**：`Ctrl+Alt+1..6` 把当前行设为对应级别标题（菜单与快捷键列表显示为「一级标题」…「六级标题」），`Ctrl+Alt+0` 清除；同级别二次按取消
- **斜体渲染间距**：预览与导出共用的排版 CSS 为 `em` 增加 `padding-right: 0.15em`，补偿斜体右倾笔画挤占后续字符的视觉间距
- 快捷键与菜单条目统一经 ShortcutManager / MenuBuilder 注册，命令面板同步获得全部条目；`Ctrl+B` 已被文件树折叠占用，行内格式与标题统一 `Ctrl+Alt+*`

**新增：脚注 / 前辅文 / 后辅文语法支持（行为变化）**

- **脚注**：`[^1]` 引用式（配对 `[^1]: 定义文本`）与 `^[行内内容]` 行内式均支持；脚注定义区自动移至文档末尾渲染（细分隔线 + 编号列表），点击角标跳转、每条定义带返回箭头，同一脚注多处引用显示 `[1:1]` 形式并各自可跳回。采用 `mdit-py-plugins` 的 footnote 插件（既有依赖，无新增）
- **前辅文**：文档开头 `---…---` YAML 块按元数据隐藏，不渲染进预览与导出（源码原样保留）；正文中间的 `---` 块不受影响。采用 `mdit-py-plugins` 的 front_matter 插件
- **后辅文**：文档末尾 `---…---` 键值对块同样隐藏（用于 `status:` / `revision:` 等尾部元数据）。业界无统一约定且无现成插件，为渲染前保守剥离：末行须为独占 `---`、内容经 PyYAML（既有依赖）解析为 dict 才隐藏；正文末尾单条水平线、非法 YAML、列表内容一律按普通正文渲染，不误删
- 预览与导出（PDF / HTML）口径一致：脚注区样式、辅文隐藏在两条渲染路径与回退渲染器上同步生效，样式全部走主题 token

**大文档预览与 PDF 导出修复（行为变化）**

- **根因**：WebView2 的 `NavigateToString` 存在 **2 MB 文档上限**（超过即真机报 `E_INVALIDARG`），而预览/导出模板恒内联 KaTeX（约 645 KB），正文可用额度只剩约 1.1 MB——正文较大的文档首屏导航直接失败，表现为**预览面板空白、PDF 导出无内容**
- **修法**：预览与 PDF 导出统一改为「先导航空壳 → 再经文档级脚本通道注入正文与按需 vendor」，不再受导航大小限制（Mermaid vendor 约 5.58 MB 本来就走该通道，现一并纳入统一路径）。所有文档一视同仁，不做大小阈值切换，避免同一份文档出现两条渲染路径
- HTML 文件导出（写盘为自包含单文件）不受影响，仍按整文档生成
- **脚注定义会让预览滚动同步"卡住"**：脚注定义区由插件移到文末渲染，但其渲染产物仍带着「定义写在源文件第几行」的源码行锚点 —— 锚点表按行号排序、按像素位置插值，一个「行号靠前、位置靠后」的反序项就会把其后整段正文压成「定义行 ~ 其后一行」，表现为预览滚过脚注引用位置后**左侧编辑器不再跟随**（定义写在中段时最容易复现）。现脚注区一律不注入源码行锚点，该区域由文末 EOF 哨兵覆盖
- **图表库注册失败会连带吞掉正文注入**（合并前审查发现）：vendor（约 5.6 MB 走 WinRT RPC）与正文脚本的两次注册原本共用一个 `try`，vendor 抛异常时正文脚本**根本没注册** —— 导航出来的空壳一直空着，导出得到正文全空的 PDF；含图表时表现为「就绪门超时、提示可能缺图表」，不含图表时连提示都没有。现两次注册各自独立兜底，vendor 失败只影响图表、正文照常注入
- **两轮导航在飞时预览会一直空白**（同一审查发现）：导航归属判定原用一个布尔标志，上一轮空壳尚未完成又发起一轮（开文件后立刻输入 / 切主题）时，第一轮完成即清掉标志，**真正上屏的第二轮**反被当成「非本适配器发起」而不发 `load_finished` —— 而内容补推只在那一次发生，预览因此一直空着。现改为**计数**，只在最后一轮完成时通知消费方
- **空壳加载完成前的推送会被补推覆盖**（同一审查发现）：模板未加载时的推送只记 `html` 而补推取的是 `_last_render_html`，异步高亮（`async_highlight` 开关）的产物不进后者，补推会把高亮结果丢掉（代码块一直无配色）。现未加载分支登记最近一次要显示的内容，补推优先用它

**合并前审查修复**

- **资源迁移撞车（可能导致丢文件）**：批量迁移同名资源时，空闲名分配只查磁盘、不查本批已分配的目标路径，后一项会覆盖前一项——而迁移是 `copy → verify → delete`，覆盖发生时源文件已被删除、内容实际丢失。现改为两遍分配（先收齐本批全部自然目标作为保留集，再逐项避让），执行前另做一次目标重复兜量检测，重复项拒绝执行并保留源文件
- **图片资源索引此前从未被写入**：`image_assets.json` 的写入路径没有生产调用者，"插图后留下位置线索"这一设计实际没生效，断链恢复只能走兜底扫描。现接到插图链路（落盘成功后登记）；写失败只记日志，不阻断插图
- **前辅文校验**：`---…---` 前辅文若不是合法 YAML 映射（如写成列表），整段正文会被静默隐藏；现照后辅文的守卫口径做校验，不合法则按普通正文渲染
- **定义列表预览/导出口径不一致**：预览注册了定义列表插件而导出没有（回退渲染器里反倒支持），同一文档两边渲染结果不同。现把任务列表 / 定义列表 / 脚注 / 前辅文统一收进 `markdown_extras` 作为唯一注册点
- **工作区外图片插不进**：图片解码走了白名单校验，而 PNG/JPEG 插入路径不校验，导致 HEIF/TIFF 等从工作区外插入必失败。现两条路径统一为同一口径
- **引用扫描超限时不再静默**：扫描超过 1 MB / 5000 篇时此前静默返回空，上层可能据此把仍在使用的图片判成孤儿。现返回"结果不可信"标记并记 warning，超限时跳过孤儿清理等破坏性操作
- **表格命令**：插入/删除行与列此前按文本反查行号，内容重复时会写错行——改用光标所在块号定位；表格识别过宽（正文里带 `|` 的行会被当成表格）——收紧为必须存在分隔行、且感知代码围栏；「格式化对齐」此前无条件重建分隔行、丢弃 `:---:` / `---:` 对齐标记——现保留已有标记，并新增**左 / 中 / 右对齐**命令（同时补上 `_table_tab_next` 不可达分支、行内格式剥离守卫）
- **光标归位**：表格编辑、任务勾选、标题设置后会写回整行文本，光标此前停在行首；现按原列号归位
- **文件树图片标签**：删除图片后标签不关闭、重命名后标签标题与路径不更新（图片标签没有 Document，旧的按文件路径匹配永不命中）；现按图片路径匹配并同步标签标题/提示
- **HTML 导出缺图可感知**：内嵌本地图片失败此前只记日志；现汇总成一条小秘书提示（最多列 3 个 + 「等 N 张」），并与「已导出」摘要**合并为一条消息**（分两条会互相覆盖）
- **打包脚本排除规则**：`EXCLUDE_ROOT` 里的多段路径（如 `data/logs`）用首段比较，**永不匹配**；且未排除运行期生成的存档 `data/gamedata/savegame.json`。现按路径前缀比较并补上存档排除
- **快捷键单一真相源**：11 个只写在菜单侧的默认键位收敛进 `_DEFAULT_SHORTCUTS`，并删除 `ShortcutManager.register()` 失效的 `default_shortcut` 参数。「跨文件搜索」不再占用 `Ctrl+Shift+F`（原有 Ctrl+Shift+P 活动栏切换已覆盖该需求）
- **行内格式包裹后预览只多出字面星号**（人工验收发现）：选区首尾带空白时（如选中 `文字 `），标记被写成 `**文字 **` —— 定界符内侧贴空白，不满足 CommonMark 的 flanking 规则，整对标记退化成**正文字符**，预览里看到的就是字面的 `**文字 **`。现标记只包住去掉首尾空白后的**核心**，空白留在标记之外（`**文字** 后面`）
- **行内格式 toggle 无法闭环**（同一验收发现）：包裹后选区丢失、光标停在「文字」与闭标记之间，再按一次同一键会插入若干星号、破坏结构；想取消只能手动把两侧星号一起选中。现包裹后**保留核心文字的选区**，再按一次即走剥壳分支；只选内部文字也生效（选区外侧紧贴同类标记时吸收**整段同类标记串**，避免斜体把 `**粗体**` 削成 `*粗体*`）；光标正贴成对标记（`**|**`、`**文字|**`）时按一次即取消该对。标记对按**从左往右两两配对**认定、只认包住光标的那一对：贴在两段之间时（`**a** **b**`）前一格的闭标记与后一格的开标记并非一对，误当一对会把相邻两段同类格式合并成 `**a b**`；选中的只是空白时等同「无选中」，光标落在标记之间而非整段末尾
- **非文件窗口的子页面可用 Esc 退出**：快捷键列表面板是独立顶层窗（非 QDialog），Esc 此前无效；现 Esc 关闭。主窗口 Esc 逐级退出：侧栏的大纲 / 跨文件搜索面板收起（**文件树除外** —— 它是文件窗口的一部分；焦点不在侧栏时也不动）→ 游戏侧栏页面回到编辑器
- **补齐「重置全部快捷键」**：此前只能逐项重置；现快捷键列表面板的标题行右侧新增「重置全部」小号纯文字按钮（无边框、次要色，避免抢眼误触），点击先确认（取消则保持不动），确认后所有键位恢复默认
- **编辑后预览会跳回旧位置**（人工验收发现）：预览侧滚动会反向驱动编辑器，该路径带抑制、不写「上次同步位置」；此后一旦因编辑（如应用行内格式）触发预览重渲染，恢复滚动时沿用了这个旧值，预览就被拽回用户已经滚过的那一段（实测可差上百行）。现恢复基准改为**编辑器当前的实时顶部行**，与「两侧顶行对齐」模型一致
- **文件树内拖拽「不问就搬」**（人工验收发现）：`QFileSystemModel` 在 `readOnly(False)` 后能自己完成落盘（rename / copy），树内拖拽因此既不询问移动还是复制、**也不做图片资源迁移**，与「拖标签页」两条路径行为不一致。现树内拖拽由视图接管，走与标签拖拽同一条请求链路（询问 → 移动 / 复制请求 → 应用层迁移），并拦掉原地放下与「文件夹拖进自己的子孙目录」。**拖文件夹是例外**：应用层入口只支持文件，接管会让「拖文件夹」变成问完就静默无动作，故文件夹仍交回模型完成目录 rename（本次接管前的能力，也不弹询问框）
- **拖标签页没有落点提示、树内拖拽又多一圈框线**（人工验收发现）：`QAbstractItemView` 只在模型 `canDropMimeData` 通过时才绘制落点指示，树内拖拽带 `text/uri-list` 天然通过、标签拖拽只带自有 MIME 被拒 —— 同一个「拖到文件夹上」的动作在文件树里有提示、拖标签时没有。而原生指示器**不可样式化**（Qt 没有 `drop-indicator` 这个可样式化的子控件，实测按系统调色板画整圈边框），还会随光标落在行中央 / 边缘在「整行框」与「一条线」之间跳，于是同一张表上两种拖拽的观感也不一致。现模型对「文件夹行 + 标签拖拽 MIME」放行**只为给出可放置的光标反馈**（落盘仍由 `dropEvent` 接管，不走模型的 `dropMimeData`），落点提示则一律关掉原生指示器、改由文件树视图按 `tree_item.drop_indicator` **自绘整行半透明高亮**（低不透明度，不遮住文字），两种拖拽只剩同一条浅色高亮
- **预览行宽改为独立选项**（人工验收发现并改口径）：编辑区「限制行宽」下一切内容都严格限制在面板宽度内、不出现横向滚动，而预览里长代码行只在代码块内横向滚动、宽表格会把整页顶宽。首版实现让预览**跟随**编辑区，但两者本就不是同一件事（编辑区是 `QPlainTextEdit` 的换行行为，预览是网页排版），想要的取值未必一致。现「设置 → 记事本设置」新增**「预览行宽」**下拉（**限制行宽 / 不限制行宽**，默认限制行宽），与「行宽模式」（编辑器自身）各自独立、各自持久化；限制行宽下预览给 `body` 加 `limit-width`（代码块折行、表格单元格可断行）
