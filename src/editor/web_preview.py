...
把预览与导出对具体 Web 控件的依赖收敛到本层之后，
使上层只依赖「能力」，而不依赖某一控件的专有 API。

后端实现：
  - WebView2：web_preview_webview2.WebView2PreviewAdapter（C3-D 起的唯一后端）

C3-D 已摘除 Qt WebEngine 后端：应用不再随包分发 Chromium，
预览与导出统一走系统共享的 WebView2 Runtime（缺失时的可见提示见
webview2_runtime.py）。

设计说明：
  - 本层抽象的是「Web 控件 API」，而非「框架」。适配器宿主在 Qt 中，
    因此接口直接使用 Qt 信号是合理的，且信号一律在主线程发出。
  - 资源根目录（能力 7）独立成方法而非 set_html 的次要参数：
    WebView2 侧它是「一次映射注册」，与「一次加载」语义不同，须由后端自行消化。
  - DPI / bounds 不在接口内：WebView2 后端需在自身内部封装 dpr 换算，
    不让上层感知（C1.6 目视检查得出的必办项）。
"""
...
def create_preview_adapter(parent: QWidget | None = None) -> WebPreviewAdapter:
    ...
    上层只应经本函数取得适配器，不得直接构造具体后端 —— 这样后端更换
    不会波及预览与导出两处调用点。
