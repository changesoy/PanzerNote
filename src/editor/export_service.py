...
完成通知：
  HTML：同步完成
  PDF：同步完成（QPdfWriter + QTextDocument.print，返回 PDF 字节）
失败通知：异常抛出 / 返回空字节
关闭时行为：无外部资源需要清理（渲染在内存缓冲区内完成）
"""

from PyQt6.QtCore import QBuffer, QIODevice, QSizeF
...
from .secure_markdown_renderer import (
    ...
    build_export_qtext_html_document,
)
...
class ExportService:
    ...
    3. HTML 导出：渲染 + 写文件
    4. PDF 导出：渲染 + QPdfWriter 生成字节，由调用方经 FileGuard 落盘
    5. 代码块语法高亮：与预览同源，固定取亮色变体配色（白纸可读）

    不在后台线程创建或操作 Qt UI 对象；QTextDocument / QPdfWriter 在主线程使用。
    """

    # QPdfWriter 光栅分辨率（dpi），同时决定页面可绘区域的像素尺寸
    PDF_RESOLUTION = 96
    ...
    @staticmethod
    def export_pdf(content: str, is_markdown: bool, colors, title: str = "",
                   theme_engine: ThemeEngine | None = None) -> bytes:
        """渲染为 PDF 字节（QTextDocument.print 同步完成）
        ...
        说明：渲染在内存缓冲区完成、不落盘；写文件由调用方经
        FileGuard.safe_write_bytes 执行，导出路径仍受文件大小限制
        与原子写入保护。
        """
        body_html = ExportService.render_content(content, is_markdown, theme_engine)
        full_html = build_export_qtext_html_document(body_html, colors, title)

        buffer = QBuffer()
        buffer.open(QIODevice.OpenModeFlag.WriteOnly)
        writer = QPdfWriter(buffer)
        writer.setPageSize(QPageSize(QPageSize.PageSizeId.A4))
        writer.setResolution(ExportService.PDF_RESOLUTION)
        if title:
            writer.setTitle(title)

        document = QTextDocument()
        document.setPageSize(
            QSizeF(writer.pageLayout().paintRectPixels(writer.resolution()).size())
        )
        document.setHtml(full_html)
        document.print(writer)

        buffer.close()
        # QBuffer.data() 返回 QByteArray；取其 .data() 得到 bytes
