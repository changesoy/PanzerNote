...
- **拖标签页没有落点指示线**（人工验收发现）：`QAbstractItemView` 只在模型 `canDropMimeData` 通过时才绘制落点线；树内拖拽带 `text/uri-list` 天然通过，标签拖拽只带自有 MIME 被拒 —— 同一个「拖到文件夹上」的动作在文件树里有线、拖标签时没有。现模型对「文件夹行 + 标签拖拽 MIME」放行（**只为画线**，落盘仍由 `dropEvent` 接管，不走模型的 `dropMimeData`），并让 `dragMoveEvent` 先经基类再接受
