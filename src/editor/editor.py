...
class Editor(QPlainTextEdit):
    ...
    def _show_context_menu(self, position):
        ...
        if self._file_type in ('JSON', 'XML', 'HTML'):
            menu.addSeparator()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if self.config.get_editor_setting("auto_pair_brackets", True):
            ...
            if char:  # 确保有输入字符
                ...
                # 情况1: 输入的是右括号/引号，检查是否应该跳过
                if char in self.AUTO_PAIR_CHARS.values():
                    if pos < len(text) and text[pos] == char:
                        # 光标后面就是这个字符，跳过而不插入
                        cursor.movePosition(cursor.Right)
                        self.setTextCursor(cursor)
                        return
                    # 新增：光标前面是对应的左括号 → 插入右括号，光标留在中间
                    close_to_open = {v: k for k, v in self.AUTO_PAIR_CHARS.items()}
                ...
                # 情况2: 输入的是左括号/引号
                if char in self.AUTO_PAIR_CHARS:
    ...
    def inputMethodEvent(self, event):
        ...
        # 情况1：输入的是右括号/引号，且光标后面就是它 -> 跳过（不再重复插入）
        if char in self.AUTO_PAIR_CHARS.values():
            if pos < len(text) and text[pos] == char:
                try:
                    event.setCommitString("", 0, 0)  # 阻止默认插入
                except Exception:
                    pass
            ...
            # 修复：某些中文输入法会在“该开引号的位置”直接提交右引号 ”/’
            if char in ("\u201d", "\u2019"):  # ” ’
                # 统计当前行到光标前的开/关引号数量，若开<=关，则此处更像“开引号”
                open_char = "\u201c" if char == "\u201d" else "\u2018"  # “ 或 ‘
                block = cursor.block()
                line_prefix = text[block.position():pos]

                # 如果正夹在两个非空白字符之间，且左右不是互相配对的符号：只插入一个中文引号（按未闭合引号栈判左右），不做成对补全
                ...
                if line_prefix.count(open_char) <= line_prefix.count(char):
                    try:
                        event.setCommitString("", 0, 0)  # 阻止默认插入这个右引号
                    except Exception:
                        pass
                    super().inputMethodEvent(event)
    ...
    def move_line_up(self):
        ...
        col = cursor.columnNumber()  # 0-based 列位置

        # 先取出文本（避免 beginEditBlock 后 block 引用失效带来的不确定性）
    ...
    def _pick_single_cjk_quote(self, ch: str, pos: int) -> str:
        ...
        # 若最近一个未闭合引号就是同类开引号，则这里更像"关引号"；否则更像"开引号"
